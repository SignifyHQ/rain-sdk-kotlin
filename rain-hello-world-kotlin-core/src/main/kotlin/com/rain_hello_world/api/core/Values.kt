...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw RainHelloWorldInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw RainHelloWorldInvalidDataException("`$name` is not set")
            is JsonNull -> throw RainHelloWorldInvalidDataException("`$name` is null")
            else -> throw RainHelloWorldInvalidDataException("`$name` is invalid, received $this")
        }

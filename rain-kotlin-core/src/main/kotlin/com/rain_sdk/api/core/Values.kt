...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw RainInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw RainInvalidDataException("`$name` is not set")
            is JsonNull -> throw RainInvalidDataException("`$name` is null")
            else -> throw RainInvalidDataException("`$name` is invalid, received $this")
        }
